...
async def test_missing_required_non_key_asks_confirm(monkeypatch):
    doctor = MagicMock()
    doctor.run = AsyncMock(return_value=EnvironmentReport(
        checks=[CheckResult(name="markitdown", status=CheckStatus.MISSING,
                            detail="not on PATH", fix_hint="pip install markitdown")],
        missing_required=["markitdown"], missing_optional=[], auto_fixable=[],
    ))
    monkeypatch.setattr("examples.pptx_generator.wizard.env.Wizard.confirm",
...
async def test_missing_optional_user_declines(monkeypatch):
    ...
    doctor.run = AsyncMock(return_value=EnvironmentReport(
        ...
        missing_required=[], missing_optional=["TAVILY_API_KEY"], auto_fixable=[],
    ))
    monkeypatch.setattr("examples.pptx_generator.wizard.env.Wizard.confirm",
                        AsyncMock(return_value=False))
    step = EnvDoctorWizardStep(doctor=doctor)
