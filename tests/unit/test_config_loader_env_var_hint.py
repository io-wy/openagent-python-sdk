...
def test_unset_env_var_includes_dotenv_hint(tmp_path, monkeypatch):
    ...
    cfg.write_text(json.dumps({"runtime": {"type": "default"}, "agents": [{"id": "a", "memory": {"type": "buffer"}, "pattern": {"type": "react"}, "llm": {"provider": "anthropic", "model": "x", "api_key": "${MY_TEST_VAR_THAT_IS_NEVER_SET}"}}]}))
